...
description: An asset definition is a description, in code, of a particular asset.
---
...
An **asset** is an object in persistent storage, such as a table, file, or persisted machine learning model. An **asset definition** is a description, in code, of an asset that should exist. It also usually describes how to update that asset.
...
- A set of upstream asset keys, which refer to assets that the contents of the asset are derived from.

The primary kind of asset definition in Dagster is a _materializable_ asset definition. A materializable asset definition includes a Python function, which is responsible for computing the contents of the asset from its upstream dependencies and storing the results.
...
The easiest way to create a materializable asset definition is with the <PyObject object="asset" decorator /> decorator.
...
By default, the name of the decorated function, `my_asset`, is used as the asset key. The decorated function is responsible for producing the asset's contents, i.e. _materializing_ it. The asset in this example doesn't depend on any other assets.
...
You can define a dependency between two assets by passing the upstream asset to the `deps` parameter in the downstream asset's `@asset` decorator. This expresses a data dependency: that the contents of the downstream asset are derived from the upstream asset. When a run is launched to materialize both of the assets, asset dependencies also determine the order of execution: an asset will always be materialized after the assets it depends on.
